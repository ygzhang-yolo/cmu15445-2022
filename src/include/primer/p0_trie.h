...
class TrieNode {
 public:
  ...
  explicit TrieNode(char key_char) {
    key_char_ = key_char;
    is_end_ = false;
    children_.clear(); // 获得一个空的children_ map
  }
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    ...
    // map中没有key_char，于是就将child作为value插入children map中(注意，unique ptr只能是移动，不能拷贝)
...
class TrieNodeWithValue : public TrieNode {
 ...
 public:
   ...
   * @param value
   */
   //即用传入的TrieNode初始化父类，并将value值给自己的成员
  // 这个初始化列表中使用std::move与std::forward是一样的，都是强调这个构造函数调用的是移动构造函数，不然会隐式调用拷贝构造函数传参
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    /*
     * 从root开始，每个的children是其下一层的字符节点们
     * TrieNode的is_end为false，证明没有到一个Key，
     * 如果想插入的<key,value>中的key与root开始到此时的部分一样，
     * 将该节点替换成value是对应value的TrieNodeWithValue即可；
     * TrieNode的is_end为true，证明从root到此是一个key,
     * 该节点存储了value,是一个TrieNodeWithValue，
     * 若此时想插入的key与其一样，则不允许插入(重复)
     * */
    if(key.empty()){
      return false;
    }
    latch_.WLock();
    // 遍历key的每个元素，查看是不是以此排在root树中
    auto c = key.begin();
    auto check = &root_;

    while(c != key.end()){
      if(check->get()->HasChild(*c)){
        // 这一层存在这个字符，都向下; check变为指向这个子节点的unique ptr的指针
        check = check->get()->GetChildNode(*c);
      }else { // 这一层没找到这个字符，需要创建这个<字符,子节点>，同时check更新为指向这个字符(子节点)的ptr的指针
        check = check->get()->InsertChildNode(*c,std::make_unique<TrieNode>(*c));
      }
      c++;
    }
    // 此时check指向树中key最后一个元素所在的节点的ptr
    if(check->get()->IsEndNode()){
      // 证明此时key与原树中的重复
      latch_.WUnlock();
      return false;
    }
    // 未重复，将此时的节点变成TrieNodeWithValue，要用new才不会在函数结束时被释放掉！
    auto tempV = new TrieNodeWithValue(std::move(*(check->get())),value);
    check->reset(tempV);
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    auto c = key.begin();
    auto check = &root_;
    std::vector<std::unique_ptr<TrieNode> *> keyNodes;
    while(c != key.end()){
      auto next = check->get()->GetChildNode(*c);
      if(next == nullptr){
        // 没找到key，false
        latch_.WUnlock();
        return false;
      }
      keyNodes.emplace_back(check);
      check = next;
      ++c;
    }
    // 此时check就指向key的结尾地方的子节点的ptr处
//    if(!check->get()->IsEndNode()){
//      // 不是key，不是结尾
//      return false;
//    }
    if(check->get()->HasChildren()){
      // 是key,但是后面还有children，因此应该将此时的TrieNodeWithValue改为无value的TrieNode,即is_end改为false
      check->get()->SetEndNode(false);
    } else{
      // 是key，且后无children，递归删除
      for(int i = keyNodes.size() - 1; i>=0 ;--i){
        auto pre = keyNodes[i];
        if(!check->get()->IsEndNode() || check->get()->HasChildren()){
          // 此时要删除的节点是其他key的结尾或者有其他children，则不能删除
          latch_.WUnlock();
          return true;
        }
        // 可以删除当前check，但是不能通过删除自己删除，要通过父亲.RemoveChildNode删除
        pre->get()->RemoveChildNode(key[i]); // 因为vector中是从root开始存的，尽管是存到结束-1，但因为多开始一个，所以i一样
        check = pre;
      }
    }
    latch_.WUnlock();
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    auto check = &root_;
    while(c != key.end()){
      auto pre = check->get()->GetChildNode(*c);
      if(pre == nullptr){
        *success = false;
        latch_.RUnlock();
        return {};
    ...
    // 是end,即TrieNodeWithValue，动态转换，看能否转换成功
    auto castNode = dynamic_cast<TrieNodeWithValue<T> *>(check->get());
    if(castNode){
      // 转换成功
      *success = true;
      latch_.RUnlock();
